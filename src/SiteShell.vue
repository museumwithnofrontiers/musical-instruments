...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('musicalInstruments.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('musicalInstruments.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('musicalInstruments.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
