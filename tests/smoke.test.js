...
describe('website smoke test', () => {
  ...
  it('renders the about page on the composed text page view', async () => {
    ...
    expect(host.textContent).toContain("Musical instruments")
  ...
  it('colours and names the source-database chip from the manifest projects section', async () => {
    const { app, host } = await mountSite('#/item/ce544eac-b951-5657-9a7f-51c32a2f26c6')
  ...
  it('shows the explore-partner notice only for the project dataset.config.js lists', async () => {
    const epm = await mountSite('#/item/a0b0c8e0-a3e5-55bb-9e5b-4ddf40ae3944')
    ...
    const isl = await mountSite('#/item/ce544eac-b951-5657-9a7f-51c32a2f26c6')
  ...
  it('renders the related-database and artistic-introduction links from the manifest', async () => {
    const [items] = await loadEntities(['items'])
    const item = items.find((i) => i.id === 'ce544eac-b951-5657-9a7f-51c32a2f26c6')
  ...
  it('renders the layout glossary tool and dynasty popouts on the item sheet', async () => {
    ...
    expect(host.textContent).toContain("Artuqids")
    app.unmount()
  }, 60000)
  ...
  it('renders the partners list on the composed partner-list view', async () => {
    ...
    expect(host.textContent).toContain('Jordan Museum for Costumes and Jewellery')
  ...
  it('renders a partner profile on the composed record view', async () => {
    const { app, host } = await mountSite('#/partner/bb94125a-04ef-5227-b3bb-8ebc7dfdafbe')
    ...
    await vi.waitFor(() => expect(host.textContent).toContain('Jordan Museum for Costumes and Jewellery'), { timeout: 20000 })
    expect(host.textContent).toContain("Amman")
  ...
  it("renders a partner's objects on the composed grid results view", async () => {
    const { app, host } = await mountSite('#/partner/bb94125a-04ef-5227-b3bb-8ebc7dfdafbe/objects')
  ...
  it('renders the shared texts and its own over them', async () => {
    ...
    expect(checkTextsRendered(host, {
      namespaces: ['musicalInstruments', 'core', 'layout', 'catalogue', 'record', 'sheet', 'timeline', 'partner', 'gallery'],
    })).toEqual([])
